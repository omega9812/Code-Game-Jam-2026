...
public class WhackAMoleGameIntegrated : MonoBehaviour
{
    ...
    [Header("Integration")]
    public ClownMinigameTrigger minigameTrigger;
    ...
    IEnumerator EndGame()
    {
        ...
        // Notify the minigame trigger that the game has ended
        if (minigameTrigger != null)
        {
            minigameTrigger.EndMinigame(win);
        }
        else
        {
            Debug.LogWarning("MinigameTrigger not assigned, cannot notify game end");
            Destroy(gameObject);
        }
    }
